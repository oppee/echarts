...
class UploadHandler
{
    ...
    protected function handle_file_upload($uploaded_file, $title, $name, $size, $type, $error,
                                          $index = null, $content_range = null)
    {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            //存入数据库
            if ($tab = $this->options['insert_tab']){
                $data = array(
                    'crdate' => time(),
                    'title' => $title,
                    'type' => $type,
                    ...
                    'info_id' => $this->options['info_id'] ? $this->options['info_id'] : 0
                );
                
                $file->id = $tab->add($data);
            }
        ...
        if ($upload && is_array($upload['tmp_name'])) {
            ...
            foreach ($upload['tmp_name'] as $index => $value) {
                $files[] = $this->handle_file_upload(
                    ...
                    $upload['name'][$index],
                    $this->getSaveName($upload['name'][$index]),

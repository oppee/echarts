...
function _ecurlopt_name($curlopt) {
...
function ecurl_setopt($ch,$option,$value) {
	...
	switch($option) {
		...
		case CURLOPT_NETRC:
		default:
			trigger_error("CURL emulation does not implement CURL option "._ecurlopt_name($option),E_USER_WARNING);
			break;
	}
}

// Perform a CURL emulation session
function ecurl_exec($ch) {
	$opt = &$GLOBALS["_CURLEXT_OPT"][$ch];
	$url = $opt["url"];
	$verbose = $opt["verbose"];
...
function ecurl_errno($ch) {
	return (int) $GLOBALS["_CURLEXT_OPT"][$ch]["errno"];
}

function ecurl_error($ch) {
	return $GLOBALS["_CURLEXT_OPT"][$ch]["error"];
}

function ecurl_getinfo($ch,$opt=NULL) {
	if ($opt) {

	...
	*/
	...
	class Calendar {
		...
		private function daysList(){
            //dump($this->month);exit;
			$out .= '<tr>';
			/* 输出空格(当前一月第一天前面要空出来) */
			for($j = 0; $j < $this->start_weekday; $j++)
				$out .= '<td>&nbsp;</td>';
			$Arr=array();	
           $datetime = M('News')->field("from_unixtime(crdate,'%Y%m%d') as crdate")->group('crdate')->select();  
			foreach ( $datetime as $key=>$value )
			   {
				if(!in_array($value['crdate'],$Arr)){
					$Arr[]=$value['crdate'];
			   }
			 }
			...
			for($k = 1; $k <= $this->days; $k++){
				$k = $k<10 ? '0'.$k : $k;				
				$thisday = $this->year.$this->month.$k;
				$j++;
				if($thisday == date('Ymd')){
				  $actiive = 'class="fontb"';  //今天的样式
				}else{
					$actiive = '';}	
					
				//$out.='<td '.$actiive.'><a href="'.U('News/search','date='.$thisday).'">'.$k.'A</a></td>';
				if(in_array($thisday, $Arr)){
					$out .= '<td '.$actiive.' class="fontmonth" ><a href="'.U('News/search','date='.$thisday).'">'.$k.'</a></td>';
				}else{
					$out .= '<td '.$actiive.'>'.$k.'</td>';
				} 
		...
		private function chageDate(){ 
			$out .= '<tr>';
			$out .= '<td class="fontyear"><a href="http://calac.demos.net.cn/news/search/date/index.php?'.$this->prevYear($this->year, $this->month).'">'.'<<'.'</a></td>';
			$out .= '<td class="fontyear"><a href="http://calac.demos.net.cn/news/search/date/index.php?'.$this->prevMonth($this->year, $this->month).'">'.'<'.'</a></td>';	
			$out .= '<td class="fontyear" colspan="3">'.$this->year.' '.'年'.$this->month.' '.'月</td>';
			$out .= '<td class="fontyear"><a href="http://calac.demos.net.cn/news/search/date/index.php?'.$this->nextMonth($this->year, $this->month).'">'.'>'.'</a></td>';
			$out .= '<td class="fontyear"><a href="http://calac.demos.net.cn/news/search/date/index.php?'.$this->nextYear($this->year, $this->month).'">'.'>>'.'</a></td>';

...
class Page
{
    ...
    protected $coolPages;
    // 分页显示定制
    protected $config = array('header' => '条记录', 'prev' => '上一页', 'next' => '下一页', 'first' => '第一页', 'last' => '最后一页', 'theme' => '<div class="total"> %totalRow% %header% %nowPage%/%totalPage% 页</div> <ul>%upPage% %downPage%%first%  %prePage%  %linkPage%  %nextPage% %end%</ul>');
    // 默认分页变量名
    protected $varPage;
        ...
        for ($i = 1; $i <= $this->rollPage; $i++) {
            ...
            if ($page != $this->nowPage) {
                ...
                if ($this->totalPages != 1) {
					$linkPage .= "<li class='active'><span class='active'>" . $page . "</span></li>";
                }
            }
        }
        $pageStr = str_replace(

...
class Image {
    ...
    //生成加减运算验证码
    static function buildOperationVerify($num = 10, $width = 80, $height = 30, $operator = array('+','-'), $verifyName = 'verify') {
        ...
        imagefilledrectangle($img, 0, 0, $width, $height-1, $black); //背景色
        ...
        if($num1 > $num2 || $num1 == $num2){
            $sign = $operator[rand(0,count($operator)-1)];
        }else{
            $sign = '+';
        }
        eval("\$randval = ".$num1.$sign.$num2.";");
        $_SESSION[$verifyName] = md5($randval);

        imagestring($img,5,5,rand(5,10),$num1, $red);
        imagestring($img,5,20,rand(5,10),$sign, $white);
        imagestring($img,5,35,rand(5,10),$num2, $green);
        imagestring($img,5,55,rand(5,10),"=", $blue);
        imagestring($img,5,65,rand(5,10),"?", $red);

...
class DbController extends BaseController {
	...
	function index() {
		...
		$page_count = count($fileArr)-2;//去除./和../
        if(I('post.limit_num') != ''){
		...
		//文件创建时间倒序
		krsort($list);
		if(isset($_GET['sorting']) && I('get.sorting')=='fileTime'){
			if(isset($_GET['order']) && I('get.order')=='asc'){
				//文件创建时间正序
				ksort($list);
			}
		}
		//切割分页输出
		$list = array_slice($list, $Page->firstRow, $Page->listRows, true);
	...
	private function setFile() {
		...
		$fileName = $this->trimPath($this->config['path'] . $this->dir_sep . $recognize . '_bak' . date('YmdHis') . '.sql');
...
//                exit;
	...
	//错误日志
	function set_log($str = 'debug message ...', $Path = '')
	{
		$dir = 'log/';
		mkdir($dir);
		$Path = $Path ? $dir.$Path : $dir.'debug_'.date('Ymd').'.txt';
		$fp=fopen($Path, "a+");
		if(is_array($str))
		{

 ...
 */
class TopicreplyController extends BaseController {

    //模型配置, 1. label, 2. tips 3. type, 4. data source, 5. special logic
    private $tca = array();
    public $tbName = "Topic_reply"; // 'link';		//MODULE_NAME
    public $image_path = '';
    public $thumb_path = '';
    // 高级搜搜状态
    ...
    protected function init() {
        ...
        if (ACTION_NAME == 'edit') {
			...
			if ($id) {
				...
				$reply = M($this->tbName)->where(array('deleted' => 0, 'id' => $id))->find();
				$users = M('User')->where(array('deleted' => 0,'hidden' => 0,'id' => $reply['user_id']))->order('id desc')->field('id as "0",concat_ws(".",id,username) as "1"')->select();
				$this->assign('users', $users);
				
				$topic = M('Topic')->where(array('deleted' => 0, 'hidden' => 0,'id' => $reply['topic_id']))->order('id desc')->field('id as "0",concat_ws(".",id,title) as "1"')->select();
				...
				$topic = M('Topic')->where(array('deleted' => 0, 'hidden' => 0))->order('id desc')->field('id as "0",concat_ws(".",id,title) as "1"')->select();
				$this->assign('topic', $topic);
			}
    ...
    public function index() {
		...
		foreach($this->status_all as $k => $v){
			if(I('request.'.$v['status']) != ''){
				...
				if(in_array($v['value'], $I_status)){
					$Where_status_info .= " or r.`".$v['status']."`='".$v['value']."' ";
        ...
        if ($category) {
            ...
            if (count($rs)) {
                ...
                $ids = implode(',', $ids);
                $Where .= " and r.category in (" . $ids . ") ";
            }
        }

        if (I('request.keyword')) {
            //$Where .=' and (r.id LIKE "%' . I('request.keyword') . '%" OR t.title LIKE "%' . I('request.keyword') . '%")';
        	$Where .=' and (t.title LIKE "%' . I('request.keyword') . '%")';
        }
        ...
        $Page = new \HQ\Util\Page($Obj->join(C("DB_PREFIX")."topic as t on t.id = r.topic_id")->where($Where)->count(), $limit_num); // 实例化分页类 传入总记录数和每页显示的记录数
        $list = $Obj->join(C("DB_PREFIX")."topic as t on t.id = r.topic_id")->field("r.*,t.title")->where($Where)->order($sorting . ' ' . $order)->limit($Page->firstRow . ',' . $Page->listRows)->select();

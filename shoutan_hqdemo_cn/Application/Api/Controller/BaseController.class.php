...
//        echo M()->getLastSql();
     ...
     * @param $order 排序方式（升序、降序）
     */
    protected function getCategory($tabName = 'news_category', $limit = "10", $hot='', $sort = "sorting", $order = "asc") {
        $Where = array(
            'deleted' => 0,
            'hidden' => 0
        );
       ...
       $objList = M($tabName)->where($Where)->order('top DESC,sorting asc')->select();
       return $objList;
    }
     ...
     * @param $category 所属分类
     */
protected function getGoodsList($limit = "", $cate = "", $hot = "", $sort = "sorting", $order = "desc",  $other = "") {
		$tbName = 'Goods';
		$Obj = D($tbName.' as g'); //实例化对象
        $Where = array(
            'g.deleted' => 0,
            'g.hidden' => 0
        );
		...
		if($hot)$Where['g.hot'] = 1;
        //$objList = $obj->where($where)->order("" . $sort . " " . $order . "")->limit("0, " . $limit)->select();
		
		$objList = $Obj->where($Where)
                            ->join(C('DB_PREFIX').'goods_category c ON c.id = g.category')
                            ->field('g.*,c.name as cate_name,c.image as cate_image')
                            ->order('g.id desc')->limit($limit)->select();

		//$objList = M('Comment')->where($Where)->join(C('DB_PREFIX').$tbName.' M ON M.id = '.C('DB_PREFIX').'comment.info_id')->field(C('DB_PREFIX').'comment.*, M.title,M.id as iid')->order(C('DB_PREFIX').'comment.sorting desc')->limit($limit)->select();
		
       // echo $Obj->getLastSql()."<br />";exit;
        //$time_now = time();
		foreach ($objList as $k => $v) {
            //是否已下架 或即将上线
            if($time_now>$v['end_time']){
                unset($objList[$k]);
            }else{
                /* if($time_now<$v['start_time']){
                    $objList[$k]['is_sale'] = 0;
                }elseif($time_now>$v['end_time']){
                    $objList[$k]['is_sale'] = 2;
                }else{
                    $objList[$k]['is_sale'] = 1;
                } */
                
                $objList[$k]['url'] = U('Goods/info', array('id' => $v['id']));

                $objList[$k]['cate_image'] = './Uploads/Goodscategory/thumb_0/'.date("Ym", $v['cate_crdate']).'/' .$v['cate_image'];
                $objList[$k]['cate_url'] = U('Goods/lists', array('id' => $v['category']));

                if (!empty($v['image'])) {
                    $path = '/Uploads/' . $tbName . '/';
                    foreach (explode(',', $this->Config['thumb_width']) as $x => $y) {
                        $thumb = $path . 'thumb_' . $x . '/' . date('Ym', $v['crdate']) . '/' . $v['image'];
                        $objList[$k]['thumb_' . $x] =  $thumb;
                    }
                    $objList[$k]['image'] = $path . 'source/' . date('Ym', $v['crdate']) . '/' . $v['image'];
                } else {
                    $objList[$k]['image'] = '__ROOT__/Web/Tpl/'.$this->Config['theme'].'/Public/images/small.jpg';
                }

                //收藏次数
                $objList[$k]['favorite'] = M('Favorites')->where('deleted=0 and hidden=0 and info_id='.$v['id'])->count();
                //评论次数
                $objList[$k]['comments'] = M('Comment')->where('deleted=0 and hidden=0 and info_id='.$v['id'])->count();
            }
		}
        return $objList;
    }
    ...
    public function getFavorite() {
        $userdata = session('FEUSER');
        $data['user_id'] = $userdata['id'];
        $data['info_id'] = $this->_post('hiddenid');
        $data['crdate'] = time();

        $favorites = M('Favorites');

        $isexist = $favorites->where("user_id=" . $data['user_id'] . " and info_id=" . $data['info_id'] . "")->find();
        if ($isexist) {
            $data['status'] = 2;
            $this->ajaxReturn($data, 'json');
        } else {
            $addid = $favorites->add($data);
            if ($addid) {
                $data['status'] = 1;
                $this->ajaxReturn($data, 'json');
            } else {
    ...
    public function delFavorite() {
        ...
        $delid = $this->_post('hiddenid');
        $favorites = M('Favorites');
        $isdel = $favorites->where("id=" . $delid . "")->delete();
        if ($isdel) {
            $data['status'] = 1;
            $this->ajaxReturn($data, 'json');
        } else {
    ...
    public function getLike() {
        ...
        $id = $this->_post('hiddenid');
        $islike = cookie('islike_' . $id);
        $music = M('Music');
        if ($islike != $id) {
            $list = $music->where("id='$id'")->setInc('likes', 1);
            $expire = strtotime("+1 months", time());
            cookie('islike_' . $id, $id, $expire);
            $num = $music->where("id='$id'")->getField("likes");
        }
     ...
     */
    function updateUserCart($user_id = 0){
        $user_id = $user_id ? $user_id : $_SESSION['FEUSER']['id'];
        if(!$user_id)return true;
        $Cart = M('Cart');
        $Goods = M('Goods');
        ...
        if($cookie_cart){
            ...
            foreach($cart_local as $k=>$v){
                ...
                $data_exist = $Cart->where($Where_exist)->find();
                if($data_exist){
                    $data_save = array(
                        'tstamp'=>time(),
                        'goods_num'=>$data_exist['goods_num']+$v['goods_num']
                    );
                    $res[] = $Cart->where($Where_exist)->save($data_save);
                }else{
                    $data_add = array();
                    $data_add['goods_id'] = $v['goods_id'];
                    $data_add['goods_num'] = $v['goods_num'];
                    $data_add['user_id'] = $user_id;
                    $data_add['crdate'] = $data_add['tstamp'] = time();
                    $res[] = $Cart->add($data_add);
                }
            }
            //清空本地购物车
            if(count($res)){
                cookie($this->Config['cookie_prefix'].'cart', null);
            }
        }
        //读取服务端购物车(db)
        $Where = array(
        ...
        foreach($data as $k=>$v){
            $Where_save = $Where;
            $Where_save['goods_id'] = $v['goods_id'];
            //根据上架时间来清除无效数据 根据购物车商品库存判断是否清除无效数据或者限制购买数量
            $v['goods_info'] = $Goods->where('deleted=0 and hidden=0 and id='.$v['goods_id'])->field('start_time,end_time,stock')->find();
            if(!$v['goods_info'] || $time_now>$v['goods_info']['end_time'] || $time_now<$v['goods_info']['start_time'] || !$v['goods_info']['stock']){
                $Cart->where($Where_save)->delete();
            }elseif($v['goods_info']['stock'] < $v['goods_num']){
                $Cart->where($Where_save)->setField('goods_num', $v['goods_info']['stock']);
            }
        }
     ...
     * @param $user_id 传入用户ID
     */
    function updateUserGold($user_id = 0){
        $user_id = $user_id ? $user_id : $_SESSION['FEUSER']['id'];
        if(!$user_id)return true;
        $SystemSend = M('SystemSend');
        //此处条件不加deleted=0,防止管理员逻辑删除后,会员再次获得积分
        $Where = 'hidden=0 and send_type=0 and user_id='.$user_id;
        $Where .= " and FROM_UNIXTIME(crdate, '%Y%m%d')=". date('Ymd');
        $exist = $SystemSend->where($Where)->find();
        //echo $SystemSend->getLastSql();exit;
        if($exist)return true;
        
        $gold_given_everyday = $this->Config['gold_given_everyday'];
        if($gold_given_everyday>0){
            $res = M('User')->where('deleted=0 and hidden=0 and id='.$user_id)->setInc('gold', $gold_given_everyday);
            if($res){
                $data_add = array(
                    'crdate' => time(),
                    'tstamp' => time(),
                    'send_type' => 0,
                    'user_id' => $user_id,
                    'gold' => $gold_given_everyday,
                    'remark' => date('Y-m-d').'登录赠送'.$gold_given_everyday.'积分'
                );
                $SystemSend->add($data_add);
            }
        }
        return true;
    }

 ...
 * @version $Id: PinCode.class.php 2.0 2012-12-23 11:36:01Z tommy $
 * @package library
 * @since 1.0
 */
namespace Unit;
...
class Verify {
    ...
    public function __construct() {
        $this->_fontSize     = 14;
        $this->_fontName     = THINK_PATH.'Library/Unit/ttf/aispec.ttf';
    ...
    protected function getPincodeContent() {
        if (!$this->_textContent) {
            $char = '2345678abcdefhijkmnpqrstuvwxyz';
    ...
    public function check($code) {
		...
		if(isset($_SESSION[$this->_sessionName]) && (strtolower($_SESSION[$this->_sessionName]) == $code)){
			session($this->_sessionName, null);
			return true;
		}
		return false;
    }
    ...
    public function show($imageUrl = null) {
        ...
        //将显示的验证码赋值给session.
        session($this->_sessionName, $this->_textContent);
        //当有headers内容输出时.
        if (headers_sent()) {
